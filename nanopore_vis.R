...
server = function(input, output, session) {
   observeEvent(input$upload, {
       ...
       ref <- input$target
       print(ref$datapath)
       file.copy(ref$datapath, "tracks/ref.fa", overwrite = TRUE)
       system(paste("samtools faidx tracks/ref.fa"))
       genomeOptions <- parseAndValidateGenomeSpec(genomeName="local",
                                                   stockGenome=FALSE,
                                                   dataMode="localFiles",
                                                   fasta="tracks/ref.fa",
                                                   fastaIndex="tracks/ref.fa.fai")
   ...
   observeEvent(input$runAlign, {
        ...
        bam <- gsub(" ", "_", paste0(sub(".f.*$", "", seqs$name), ".bam"))
        system(paste0("minimap2 -ax splice tracks/ref.fa '", seqs$datapath, "' | samtools sort - -o tracks/", bam, "; samtools index tracks/", bam))
        ...
        ## x <- GenomicAlignments::readGAlignments(paste0("tracks/", bam), param = Rsamtools::ScanBamParam(what="seq"))
        ## loadBamTrackFromLocalData(session, id="igvShiny_0", trackName="tumor.bam", data=x)

        message <-
            list(
                elementID = "igvShiny_0",
                trackName = "align",
                bamDataFilepath = paste0("tracks/", bam),
